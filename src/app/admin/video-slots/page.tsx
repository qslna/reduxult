...
export default function VideoSlotsPage() {
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(['designers', 'fashion-films']));
  ...
  const allSlotsGrouped = useMemo(() => {
    return videoSlotUtils.getAllSlotsGrouped().map(({ page, slots }) => ({
      page,
    ...
    return allSlotsGrouped.map(({ page, displayName, slots }) => ({
      ...
      slots: slots.filter(slot => {
        ...
        const matchesSearch = !searchQuery || 
          ...
          slot.currentVideo?.title?.toLowerCase().includes(searchQuery.toLowerCase());

        // Type filter
        const matchesFilter = (() => {
          switch (filterType) {
            case 'with-videos':
              return !!slot.currentVideo;
            case 'empty':
              return !slot.currentVideo;
            case 'errors':
              return !!slot.error;
            case 'google-drive':
              return slot.currentVideo?.provider === 'google-drive';
            case 'youtube':
              return slot.currentVideo?.provider === 'youtube';
            case 'upload':
              return slot.currentVideo?.provider === 'upload';
            case 'all':
            default:
              return true;
          }
  ...
  function getPageDisplayName(pageId: string): string {
    const displayNames: Record<string, string> = {
      'home': 'Home Page',
      'designers': 'Designer Showcases',
      'fashion-films': 'Fashion Films Gallery',
  ...
  const getProviderIcon = (provider?: string) => {
    switch (provider) {
      ...
      case 'vimeo':
        return <VideoIcon className="w-4 h-4 text-cyan-400" />;
      case 'upload':
        return <FileVideo className="w-4 h-4 text-green-400" />;
      default:
        return <Globe className="w-4 h-4 text-gray-400" />;
    }
  };

  const handleSlotUpdate = (slot: VideoSlot, newVideo: VideoSlot['currentVideo']) => {
    store.updateSlotVideo(slot.id, newVideo);
  };

  const handleSlotDelete = (slot: VideoSlot) => {
    store.deleteSlotVideo(slot.id);
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div className="bg-gradient-to-br from-green-500/20 to-emerald-500/20 rounded-2xl p-6 border border-green-400/30">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-white">{stats.slotsWithVideos}</div>
                    <div className="text-green-300 text-sm">With Videos</div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(stats.providerCounts).map(([provider, count]) => (
                  <div key={provider} className="flex items-center gap-3 p-3 bg-white/5 rounded-xl border border-white/10">
                    {getProviderIcon(provider)}
                    <div>
                      <div className="text-white font-semibold">{count}</div>
                      <div className="text-white/60 text-sm capitalize">{provider}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div className="flex flex-col sm:flex-row gap-4 flex-1">
              ...
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search video slots..."
                  value={searchQuery}
        ...
        <div className="space-y-6">
          {filteredSlotsGrouped.length === 0 ? (
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-12 text-center border border-white/10">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">No matching video slots found</h3>
              ...
              >
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="px-3 py-1 bg-red-500/20 text-red-300 text-sm rounded-full border border-red-400/30">
                  ...
                  <div className="flex items-center gap-4 text-sm text-white/60">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{slots.filter(s => s.currentVideo).length} with videos</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <AlertCircle className="w-4 h-4" />
                      <span>{slots.filter(s => !s.currentVideo).length} empty</span>
                    </div>
                    {/* Provider breakdown */}
                    <div className="flex items-center gap-1">
                      {slots.filter(s => s.currentVideo?.provider === 'google-drive').length > 0 && (
                        <Monitor className="w-4 h-4 text-blue-400" />
                      )}
                      {slots.filter(s => s.currentVideo?.provider === 'youtube').length > 0 && (
                        <Youtube className="w-4 h-4 text-red-400" />
                      )}
                      {slots.filter(s => s.currentVideo?.provider === 'upload').length > 0 && (
                    ...
                    <div className={
                      ...
                      {slots.map((slot) => (
                        ...
                        >
                          <VideoSlotManager
                            slot={slot}
                            onVideoUpdate={handleSlotUpdate}
                            onVideoDelete={handleSlotDelete}
